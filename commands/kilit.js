...
exports.run = (client, message, args) => {
  ...
  if (validUnlocks.includes(time)) {
    ...
    message.channel.overwritePermissions(message.guild.id, {
      ...
      message.channel.send(`**Kanal Kilitlendi.<a:tik:750997694276698173> ** ${ms(ms(time), { long:true })}`).then(() => {
        client.lockit[message.channel.id] = setTimeout(() => {
          message.channel.overwritePermissions(message.guild.id, {
            SEND_MESSAGES: null
          }).then(message.channel.send('**Kanalın Kilidi Açıldı.<a:tik:750997694276698173> **')).catch(console.error);
          delete client.lockit[message.channel.id];
        }, ms(time));
      }).catch(error => {
        console.log(error);
